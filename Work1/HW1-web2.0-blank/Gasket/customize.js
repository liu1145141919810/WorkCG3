...
window.onload = function init()
{
    ...
    _recursion_( vertices[0], vertices[1],numTimesToSubdivide);//细分入口
...
function _recursion_( a, b,count )//递归调用处
{
    ...
    if ( count == 0 ) {
        ...
        //bisect the sides找中点
        count=count-1;        //count--;	
        var vector= subtract(b,a);
        var v1,v2;
        if(vector[1]==0){
            //it's a good solution 
            v1=vec2(1*vector[0]/2,0);
            v2=vec2(0,-1*vector[0]/2);
        }
        else{
            v1=vec2(0,vector[1]/2)
            v2=vec2(vector[1]/2,0)
        }
        var a1=vec2(a[0]+v1[0],a[1]+v1[1]);
        var a2=vec2(a1[0]+v2[0],a1[1]+v2[1]);
        var a3=vec2(a2[0]+v1[0],a2[1]+v1[1]);
        ...
        console.log(b)
        console.log("============")
        _recursion_( a, a1,count );
		_recursion_( a1,a2,count );
        _recursion_( a2,a3,count );
        _recursion_( a3,b,count );
...
function  countFPS() {
    ...
    if (now > 1000 + lastTime) {
		...
		document.getElementById("label").innerHTML=("FPS_: "+fps);
        frame = 0;
        lastTime = now;
    };
}